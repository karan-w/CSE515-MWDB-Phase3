...
def ppr(teleportation_matrix, random_walk, len_seed):
...
for i in range(len(test_images)):
    teleportation_matrix, tele_np = compute_seed_matrix(label_images_list=[test_images_names[i]], n=len(random_walk),
    ...
    df = pd.DataFrame(ppr(tele_np, random_walk, 1),columns=["ppr"])
